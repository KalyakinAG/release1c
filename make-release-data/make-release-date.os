...
Функция ПолучитьОписаниеЗадачи(Задача, ЕстьВРелизе = Истина)
	...
	ОписаниеЗадачи.Вставить("Комментарии", Новый Массив);
	comment = Поля["comment"];
	Если comment <> Неопределено Тогда
		comments = comment["comments"];
		Если ЗначениеЗаполнено(comments) Тогда
			Для каждого comment Из comments Цикл
				Комментарий = Новый Структура("Автор, Содержание, ДатаСоздания, ДатаОбновления", comment["author"]["name"], comment["body"], comment["created"], comment["updated"]);
				ОписаниеЗадачи.Комментарии.Добавить(Комментарий);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(РабочийКаталог) Тогда
		...
		Настройка.Вставить("workingDir", РабочийКаталог);
	КонецЕсли;
	//  Чтение настроек
	...
	|""components""
	|],
	|""jql"": ""Спринт in openSprints() AND project = %1 AND fixVersion = %2""
	|}", Проект, releaseVersion);

	//|""customfield_11202"", - поле Разработчик
	//|""customfield_10105"", - поле Спринт
	...
	Для каждого Задача Из issues Цикл
		...
		Если ОписаниеЗадачи.Свойство("Метки", Метки) Тогда
			Если Метки.Найти("Описание_релиза") <> Неопределено Тогда
				...
				Данные.Вставить("ОписаниеВерсии", Задача["fields"]["fixVersions"][0]["description"]);
				Данные.Вставить("Комментарии", ОписаниеЗадачи.Комментарии);
				Продолжить;
			КонецЕсли;
			Если Ложь
				...
				ИЛИ Метки.Найти("Внешняя_обработка") <> Неопределено
				ИЛИ Метки.Найти("Внешний_отчет") <> Неопределено
			Тогда
...
Приложение.Версия("v version", "2.0.3");
